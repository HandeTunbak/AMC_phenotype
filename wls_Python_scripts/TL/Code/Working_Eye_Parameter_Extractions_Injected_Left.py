...
> scipy:        $ conda install scipy                                          - click y when prompted
> imageio:      $ conda install imageio                                        - click y when prompted
> matplotlib:   $ conda install matplotlib                                     - click y when prompted
> pandas:       $ conda install pandas                                         - click y when prompted
> plotly:       $ conda install plotly                                         - click y when prompted
> skimage:      $ conda install scikit-imageconda install skimage              - click y when prompted
...
# Experiment_condition = '/Control/**/Original - Copy' 
Experiment_condition = '/Injected/**/Original - Copy' 
...
#%% # manipulate images 
    ...
    bright_img = brighten.enhance(3.65)
    # bright_img.show()
    ...
    contrast_img = up_contrast.enhance(5.8)
    # contrast_img.show()
    ...
    contrast_img_2 = up_contrast_2.enhance(1)
    # contrast_img_2.show()
...
# if fish facing the left, the eye has the smallest X value. 
    ...
    fish_IDs= pd.concat([fish_IDs, fish_ID], axis=0)


    

# #%%   # Summarise findings on one table 
fish_IDs.columns=['Fish_IDs']

if filename == 'image_paths_left':   
    left_eye_parameters_all = pd.DataFrame(eye_parameters_all).T
    left_eye_parameters_all2 = left_eye_parameters_all.insert(loc=0, column='fish_IDs', value= fish_IDs['Fish_IDs'])
    left_eye_parameters_all.to_csv( str(Path(img_paths_left[f]).parents[2]) + '/' + condition + '_Left_Eye_Parameters.csv', index=False)
   
    
elif filename == 'image_paths_right': 
    right_eye_parameters_all = pd.DataFrame(eye_parameters_all).T
    right_eye_parameters_all2 = right_eye_parameters_all.insert(loc=0, column='fish_IDs', value= fish_IDs['Fish_IDs'])
    right_eye_parameters_all.to_csv( str(Path(img_paths_right[f]).parents[2]) + '/' + condition + '_Right_Eye_Parameters.csv', index=False)
