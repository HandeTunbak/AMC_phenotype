...
#-----------    
elif condition == 'Injected' and direction == 'Rrght':
...
for f, fish_imgs in enumerate(img_paths):
    ...
    cleaned_mask = remove_small_objects(mask, min_size=100, connectivity=4)
    ...
    centroids_table2= centroids_table2.drop([7])
    ...
    fish_length= sum(distances_all[0])
    fish_length= pd.DataFrame([fish_length])
    Fish_body_lengths_all= pd.concat([Fish_body_lengths_all, fish_length])
...
Fish_body_lengths_all = pd.concat([Fish_IDs_all, Fish_body_lengths_all], axis=1)
    

#%%

if condition=='Control' and direction== 'Left':   
    Fish_body_lengths_all.to_csv( str(Path(img_paths[f]).parents[2]) + '/' + condition + '_Left_Body_Length.csv', index=False)


elif condition=='Control' and direction== 'Right':   
    Fish_body_lengths_all.to_csv( str(Path(img_paths[f]).parents[2]) + '/' + condition + '_Right_Body_Length.csv', index=False)
