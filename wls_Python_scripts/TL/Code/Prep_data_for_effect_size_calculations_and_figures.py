...
for file in csv_file_paths:
    ...
    Control = pd.DataFrame(index=range(csv_data['Control'].count()),columns=range(4))
    Control.columns = ['Condition', 'Genotype', 'Treatment', 'Value']
    Control['Condition'] = 'Control'
    Control['Genotype'] = 'mab_sibs'
    Control['Treatment'] = 'none'
    Control['Value'] = csv_data['Control']
    Control = Control.reset_index(drop=True)
        
    Injected = pd.DataFrame(index=range(csv_data['Injected'].count()),columns=range(4))
    Injected.columns = ['Condition', 'Genotype', 'Treatment', 'Value']
    Injected['Condition'] = 'Test'
    Injected['Genotype'] = 'mab_sibs'
    Injected['Treatment'] = 'wls-/-'
    Injected['Value'] = csv_data['Injected']
    Injected = Injected.reset_index(drop=True)
    
    # Creat dataframe to store the data
    df_list= [Control, Injected]
