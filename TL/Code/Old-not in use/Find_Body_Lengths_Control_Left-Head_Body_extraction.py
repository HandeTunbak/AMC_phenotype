...
for f, fish_imgs in enumerate(img_paths):
    ...
    if len(distances_all) ==8:
        print ('measurements: 8 of 8' )
    else: 
        print ('measurements: '+ str(len(distances_all))  + ' of 8' )
    
    fish_head_length= sum(distances_all[0][0:3])
    fish_head_length= pd.DataFrame([fish_head_length])
    
    fish_tail_length= sum(distances_all[0][3:8])
...
if condition=='Control' and direction== 'Left':   
    Fish_body_lengths_all.to_csv( str(Path(img_paths[f]).parents[2]) + '/' + condition + '_Left_Body_Parameters.csv', index=False)


elif condition=='Control' and direction== 'Right':   
    Fish_body_lengths_all.to_csv( str(Path(img_paths[f]).parents[2]) + '/' + condition + '_Right_Body_Parameters.csv', index=False)
