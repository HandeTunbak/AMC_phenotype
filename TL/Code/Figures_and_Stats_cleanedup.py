...
# Ensure that index labels are unique
...
# Extend both series to the maximum length
s1 = s1[:max_length]
s2 = s2[:max_length]

# Concatenate Series along columns, filling missing values with NaN
data_df = pd.concat([s1, s2], axis=1)

# Ensure consistent data types (e.g., converting all values to float)
data_df = data_df.astype(float)

# Strip leading/trailing whitespace (if needed)
data_df = data_df.apply(lambda x: x.map(lambda y: y.strip() if isinstance(y, str) else y))

data_df.columns=['Control', 'Injected']

# Save 
folder_4_saving= 'C:/Users/hande/OneDrive - Oxford Brookes University/Documents/Projects/WLS/Code_testing/WLS_TL/Testing_area/Effect_size_prep/'
file_name= ('Eye_length_(um).csv')

data_df.to_csv(str(str(folder_4_saving + file_name)), index=False)
...
plt.savefig( Experiment_folder + '/Figures_and_Stats' + '/All_Eye_Lengths-Boxplot_with_scatter.eps', dpi=900)  
plt.show()    
...
plt.savefig( Experiment_folder + '/Figures_and_Stats' + '/All_Eye_Areas-Boxplot_with_scatter.eps', dpi=900)  
plt.show()  
...
# Control
s1= pd.concat([Control_Left_Body_Length, Control_Right_Body_Length ], axis=1)
s1= s1 * Body_conversion_3x    
s1= np.mean(s1, axis=1)

# Injected
s2= pd.concat([Injected_Left_Body_Length, Injected_Right_Body_Length ], axis=1)
...
#%% Save data Body length

# Control
s1= Combined_Control_Body_Length
s1= s1 * Body_conversion_3x  

# Injected
s2= Combined_Injected_Body_Length
s2= s2 * Body_conversion_3x  
...
# Save 
folder_4_saving= 'C:/Users/hande/OneDrive - Oxford Brookes University/Documents/Projects/WLS/Code_testing/WLS_TL/Testing_area/Effect_size_prep/'
file_name= ('Body_length_(um).csv')

data_df.to_csv(str(str(folder_4_saving + file_name)), index=False)

#%% Save data Average_Body length
...
# Save 
folder_4_saving= 'C:/Users/hande/OneDrive - Oxford Brookes University/Documents/Projects/WLS/Code_testing/WLS_TL/Testing_area/Effect_size_prep/'
file_name= ('Average_Body_length_(um).csv')
...
plt.savefig( Experiment_folder + '/Figures_and_Stats' + '/Average_Body_Lengths-Boxplot_with_scatter.eps', dpi=900)  
plt.show()    
...
plt.savefig( Experiment_folder + '/Figures_and_Stats' + '/All_Eye_Eccentricity-Boxplot_with_scatter.eps', dpi=900)     
plt.show()  
...
plt.savefig( Experiment_folder + '/Figures_and_Stats' + '/All_Eye_Body_Ratio-Boxplot_with_scatter.eps', dpi=900)       
plt.show()  
...
plt.savefig( Experiment_folder + '/Figures_and_Stats' + '/All_Head_Body_Ratio-Boxplot_with_scatter.eps', dpi=900)       
plt.show()  
...
plt.savefig( Experiment_folder + '/Figures_and_Stats' + '/Eye_Head_Ratio.eps', dpi=900)    
plt.show()  
