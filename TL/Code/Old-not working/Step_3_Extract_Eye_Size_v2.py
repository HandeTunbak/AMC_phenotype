...
left_eye_features = np.zeros((len(img_paths_left), 8))

right_eye_features = np.zeros((len(img_paths_right), 8))
...
#%% # manipulate images 
    ...
    # brighten image
    PIL_image = Image.fromarray(cropped_img, 'RGB')

    brighten = ImageEnhance.Brightness(PIL_image)
    bright_img = brighten.enhance(2)
    # bright_img.show()
    ...
    up_contrast = ImageEnhance.Contrast(PIL_image)
    contrast_img = up_contrast.enhance(8.9)
    # contrast_img.show()
    ...
    bright_img_2 = brighten_2.enhance(6.5)
    # bright_img_2.show()
    ...
    contrast_img_2 = up_contrast_2.enhance(10)
    # contrast_img_2.show()
    ...
    contrast_img_2.save(fish_imgs[:-4]  + '_enhanced.jpg' )
    img2 = io.imread(fish_imgs[:-4] + '_enhanced.jpg')
...
#-------- 
    ...
    # remove small particles from image
    cleaned_img_eye_2 = remove_small_objects(dilated_eye_mask, min_size=4400, connectivity=2)
...
#--------  
 # Label areas in mask
    ...
    fig = px.imshow(cleaned_img_eye_2, binary_string=True)
    ...
    if filename == 'image_paths_left':
        ...
        sorted_df_eye = pd.DataFrame(props_2).sort_values(by='centroid-0', ascending=True)
        eye_properties = sorted_df_eye.loc[0]
        eye_properties = eye_properties.rename_axis('').reset_index()
        eye_properties = eye_properties.T
        eye_properties.columns = eye_properties.iloc[0]
        eye_properties = eye_properties[1:]
        left_eye_features[f,:] = eye_properties
    ...
    # if fish facing the right, the eye has the largest X value.         
        ...
        sorted_df_eye = pd.DataFrame(props_2).sort_values(by='centroid-0', ascending=False)
        eye_properties = sorted_df_eye.loc[0]
        eye_properties = eye_properties.rename_axis('').reset_index()
        eye_properties = eye_properties.T
        eye_properties.columns = eye_properties.iloc[0]
        eye_properties = eye_properties[1:]
        right_eye_features[f,:] = eye_properties
    ...
    fig, ax = plt.subplots()
    ax.imshow(dilated_eye_mask, cmap=plt.cm.gray)
...
#%%   # Give update on progress 
    if filename == 'image_paths_left': 
        print('Completed ' + str(f+1) + ' of ' + str(len(img_paths_left)))
        
    elif filename == 'image_paths_right': 
        print('Completed ' + str(f+1) + ' of ' + str(len(img_paths_right)))
        
    else:
        print ('Error providing progress')
...
if filename == 'image_paths_left': 
    left_eye_features =  pd.DataFrame(left_eye_features)
    left_eye_features.columns = [ 'Centroid X axis',
                                  'Centroid Y axis',
                                  'Area',
                                  'Perimeter',
                                  'Orientation',
                                  ...
                                  'Minor_axis_length',
                                  'Label'                 ]
    left_eye_features.to_csv( str(Path(img_paths_left[f]).parents[1]) + '\Left_eye_measurements_pixels.csv')
    
elif filename == 'image_paths_right': 
    right_eye_features =  pd.DataFrame(right_eye_features)
    right_eye_features.columns = ['Centroid X axis',
                                  'Centroid Y axis',
                                  'Area',
                                  'Perimeter',
                                  'Orientation',
                                  ...
                                  'Minor_axis_length',
                                  'Label'               ]
    right_eye_features.to_csv( str(Path(img_paths_right[f]).parents[1]) + '\Right_eye_measurements_pixels.csv')        
